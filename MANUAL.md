...
| `vision_api_key_env` | `string \| null` | `null` | Override for the variable holding the vision API key. When unset, each provider uses its own: `GOOGLE_API_KEY` or `GEMINI_API_KEY` (Google's precedence), `OPENAI_API_KEY`, or `ANTHROPIC_API_KEY` (Claude also accepts an `ant auth login` profile). Keys may also sit in a `.env` file in the working directory or a parent; exported variables take precedence. Claude requires `pip install 'pcb-inspector[claude]'`. |
